...
Notedown Sync Server는 Notedown 앱의 워크스페이스, 노트 메타데이터, 마크다운 문서 파일을 여러 기기에서 동기화하기 위한 서버입니다. 서버는 파일별 `serverRevision`, 클라이언트가 마지막으로 확인한 `lastKnownRevision`, 콘텐츠 해시를 비교해 충돌을 감지하고, 자동 덮어쓰기가 위험한 경우 충돌 응답을 반환합니다.

### 주요 기능

- Notedown 메타데이터와 개별 마크다운 파일 동기화
- `storage/metadata.db` SQLite 저장소 기반 메타데이터 관리
...
- `storage/metadata.db`: 서버가 보유한 Notedown 메타데이터 SQLite 저장소
- `storage/files/`: 동기화된 마크다운 파일과 내부 Git 저장소. 파일은 워크스페이스 이름, 노트 제목, 첨부 파일명을 기반으로 읽기 쉬운 `storagePath`에 저장됩니다.
...
2. 전체 동기화가 필요하면 `/api/sync/plan`에 로컬 메타데이터와 `knownFiles`를 보냅니다.
...
Notedown Sync Server is a Flask-based synchronization service for Notedown workspaces, note metadata, and markdown note files. It compares each file's `serverRevision`, the client's `lastKnownRevision`, and content hashes to detect conflicts before overwriting remote or local changes.

### Features

- Synchronizes Notedown metadata and markdown files
- Stores server metadata in `storage/metadata.db` SQLite tables
...
- `storage/metadata.db`: server-side Notedown metadata SQLite store
- `storage/files/`: synchronized markdown files and the internal Git repository. Files use readable `storagePath` values based on workspace names, note titles, and attachment names.
...
2. For a full sync, send local metadata and `knownFiles` to `/api/sync/plan`.
